...
Inicalmente vamos dividir o dataset entre o dataset de treino e validação. Essa divisão é importante por evita o overfitting que ocorre quando um modelo estatístico super se adpta ao conjunto treinado, dessa forma quando o modelo recebe um valor pelo o qual ele não foi treinado, ele vai gerar um predição muito ruim. É importante essa divição entre treino e validação para verificar em qual ponto o modelo começa a sofrer overfitting. 
...
A minha estrategia é primeiro acahar o melhor modelo que encontre a quatidade diária que será vendida para cada produto, e só depois encontrar o melhor modelo que encontre o preço diário desse produto.
...
Para verificar qual dos 3 modelos é o melhor utilizamos a metrica Rsquared, onde o valor do Rsquared (entre 0 e 1) é o percentual de variância explicada pelo o modelo. Quanto maior foi o Rsquared melhor é o modelo.
...
É possível notar que o valor de Rsquared do treino sofreu uma ligeira queda, porém o valor do Rsquared sofreu um aumento. Dessa forma a gente evita o overfitting
...
* Rodar RF com 50 árvores e 30 de profundidade (Esse número deveria ser bem mais alto, porém por motivos de processamento esse valores foram escolhidos)
* Encontrar o ponto onde o modelo começa a ficar super especialista (overfitthing)
...
Para esse novo modelo não foi possível notar o ponto onde o modelo começa a ficar super especialista. Por esse motivo foi criado um novo modelo, dessa vez utilizando 100 árvores e 30 de profundidade. 
...
É possível notar que depois da árvore 60, tanto o treinamento quando a validação praticamente se estabilizam. Por esse motivo, o número de árvores ideal para esse conjunto de dados é 60 (com mais árvores só estaríamos "desperdiçando" processamento para ter pouca melhora no modelo). 
...
É possível notar que o valor do Rsquared subiu em relação ao modelo anterior. Devemos agora investigar quais foram as variáveis que mais contribuiram essa mudança.
<div style="text-align:center" markdown="1">
![treinamento](/home/rodolfo/Projetos/Desafio-Precificacao/Scripts/plots/RF_60_30_variable_importance.png)
</div>
Notamos que as três novas variáveis adicionadas se encontram no Top 5 das variáveis que mais contribuiram para a criação do modelo. Podemos afimar então que as novas variáveis influênciaram possitivamente o modelo.
...
É possível notar que o após 17 árvores o modelo se torna super especialista. Por esse motivo foi criado um novo modelo com apenas 17 árvores. Por essa razão criamos um novo modelo, dessa vez com apenas 17 árvores
...
É interessante notar que o concorrente C1 se encontra no Top 3 das variáveis que mais constribuiram para a criação do modelo. Esse mesmo concorrente foi destacado como sendo o concorrente que possui os melhores preços antes mesmo da criação de qualquer modelo
...
* Dia_Mes - Mostra se o dia se encontra no começo, meio e fim do mês 
